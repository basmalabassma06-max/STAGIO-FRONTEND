  ...
  <Teleport to="body">
    <div v-if="show" class="modal-overlay" @click.self="$emit('cancel')">
      <div class="modal">
        ...
        <div v-if="withReason" class="form-group" style="margin-bottom:16px">
        ...
        <div class="modal__actions">
          ...
          <button :class="['btn', danger ? 'btn--danger' : 'btn--primary']" @click="$emit('confirm', reason)">{{ label }}</button>
        </div>
      </div>
    </div>
  </Teleport>
...
.modal { background:var(--bg2); border:1px solid var(--border); border-radius:14px; padding:28px; max-width:440px; width:90%; }
